...
router.post("/", async (req, res) => {
  ...
  if (req.body.author === post.userId) {
    return res.status(403).json("You can't comment your own post.");
  }
  ...
  //   return res
  //     .status(403)
  //     .json("Complete transaction is required for commenting.");
  // }
  ...
  if (!req.body.rating) {
    return res.status(403).json("Rating is required.");
  }
...
router.put("/:id", async (req, res) => {
  try {
    ...
    if (req.body.author !== comment.author.toString()) {
      return res.status(403).json("You can't update this comment.");
    }
